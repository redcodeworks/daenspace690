...
abstract class NlpPipeline {
  ...
  val docAssembler = new DocumentAssembler() setInputCol("description") setOutputCol("document")
...
abstract class StandardPipeline extends NlpPipeline {
  lazy val tokenizer = new Tokenizer() setInputCols(docAssembler.getOutputCol) setOutputCol("token")
  lazy val normalizer = new Normalizer() setInputCols(tokenizer.getOutputCol) setOutputCol("normalized")
  lazy val stopwordsCleaner = new StopWordsCleaner() setInputCols(normalizer.getOutputCol) setOutputCol("cleanedTokens")
  lazy val lemma = new LemmatizerModel(config.getString("pipeline.lemmatizerModel")) setInputCols(stopwordsCleaner.getOutputCol) setOutputCol("lemma")

  val wordEmbeddings: Transformer
  val sentenceEmbeddings: Transformer

  val pipeline: Pipeline = new Pipeline().setStages(
    Array(docAssembler, tokenizer, normalizer, stopwordsCleaner, lemma, wordEmbeddings, sentenceEmbeddings, classifier)
  )
}
...
object GlovePipeline extends StandardPipeline {
  lazy val wordEmbeddings: WordEmbeddingsModel = WordEmbeddingsModel.pretrained()
...
object BertPipeline extends StandardPipeline {
  lazy val wordEmbeddings = BertEmbeddings.pretrained(config.getString("bert.model"), lang)
      .setInputCols(docAssembler.getOutputCol, lemma.getOutputCol)
      .setOutputCol("embeddings")
...
object ElmoPipeline extends StandardPipeline {
  lazy val wordEmbeddings = ElmoEmbeddings.pretrained(config.getString("elmo.model"))
      .setInputCols(docAssembler.getOutputCol, lemma.getOutputCol)
      .setOutputCol("embeddings")
